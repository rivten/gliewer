...
// NOTE(hugo) : This code is largely taken directly from the sample code of the ImGui project
...
void ImGuiNewFrame(SDL_Window* window, game_input* Input)
{
    if (!g_FontTexture)
    ...
    if (SDL_GetWindowFlags(window) & SDL_WINDOW_MOUSE_FOCUS)
        io.MousePos = ImVec2((float)Input->MouseX, (float)Input->MouseY);   // Mouse position, in pixels (set to -1,-1 if no mouse / on another screen, etc.)
    else
        io.MousePos = ImVec2(-1, -1);

    io.MouseDown[0] = Input->MouseButtons[0].EndedDown;
    io.MouseDown[1] = Input->MouseButtons[2].EndedDown;
